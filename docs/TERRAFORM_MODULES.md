...
Provisions, for CareToCoin: a per-app CockroachDB database, a least-privilege
...
module "helixchain" {
  ...
  app_name     = "caretocoin"
  db_password  = var.caretocoin_db_password   # TF_VAR_caretocoin_db_password
  cluster_host = var.cluster_host
  grants = {
    identities  = ["SELECT"]
    credentials = ["SELECT"]
    rwa_objects = ["SELECT", "INSERT"]           # care tokens / assets
    ledger      = ["SELECT", "INSERT"]           # value movement
  }
}
```
