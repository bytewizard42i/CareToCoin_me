          ...
          return (
            <div key={item.id}
                 className={`rounded-xl border p-3 transition ${
                   selected ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 ring-1 ring-indigo-300' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-slate-300'
                 }`}>
              <div className="flex items-start gap-3">
                <button onClick={() => onSelect(item.id)} className="flex flex-1 items-start gap-3 text-left">
                  ...
                  <span className="flex-1">
                    ...
                    {renderMeta && <span className="mt-0.5 block text-xs text-slate-500 dark:text-slate-400">{renderMeta(item)}</span>}
                  </span>
                </button>
                <button
                  onClick={() => setResearchId(open ? null : item.id)}
                  title="Research this option"
                  className={`rounded-md p-1.5 ${open ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'text-slate-400 hover:bg-slate-100'}`}>
                  <Info size={16} />
                </button>
              </div>
